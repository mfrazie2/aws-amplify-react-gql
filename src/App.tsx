...
const App = ({ signOut }: WithAuthenticatorProps) => {
  ...
  async function fetchNotes() {
    ...
    Promise.all(notesFromAPI.map(async (note: Note) => {
      if (note.image) {
  ...
  async function createNote(event: any) {
    ...
    const image: FormDataEntryValue | null = form.get('image');
    const imageName = (image as File).name
    const name = form.get('name') ?? 'default name';
    const data = {
      name,
      description: form.get("description"),
      image: imageName ?? '',
    };
    if (!!data.image) Storage.put(name as string, image);
    await API.graphql({
